...
import { Calendar, Lock, Mail, User, Building2 } from 'lucide-react'

interface SignupScreenProps {
  onSignup: (name: string, email: string, password: string, practiceName: string) => void
  onLogin: () => void
}
...
export function SignupScreen({ onSignup, onLogin }: SignupScreenProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate API call
    setTimeout(() => {
      onSignup(name, email, password, practiceName)
      setIsLoading(false)
    }, 800)
  }
        ...
        <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl p-8 border border-white/20">
          <div className="mb-6">
            ...
            <p className="text-sm text-slate-500">
              Create your account and set up online appointments
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              <Label htmlFor="practiceName" className="text-sm font-medium text-slate-700">
                Practice name
              </Label>
              <div className="relative">
